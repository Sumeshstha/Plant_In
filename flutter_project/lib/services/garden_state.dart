...
class GardenState with ChangeNotifier {
  ...
  void _initializeData() {
    ...
    // Build default plants with simulation care scheduling initialized to 2026-06-15
    _plants = [
      Plant(
        id: '1',
        ...
        journals: [
          JournalEntry(
            ...
            date: '2026-06-10',
            category: 'watering',
            title: 'Weekly watering completes',
            notes: 'Leaves dusted, compost moist.',
          ),
        ],
        lastWateredDate: '2026-06-10',
        wateringIntervalDays: 7,
        lastFertilizedDate: '2026-06-01',
        fertilizerIntervalDays: 30,
        lastRepottedDate: '2025-06-01',
      ...
      Plant(
        ...
        wateringIntervalDays: 7,
        lastFertilizedDate: '2026-06-05',
        fertilizerIntervalDays: 14,
        lastRepottedDate: '2026-02-14',
        repotIntervalMonths: 12,
      ),
      Plant(
        id: '3',
        ...
        spaceId: 's1',
        journals: [],
        lastWateredDate: '2026-06-12',
        wateringIntervalDays: 5,
        lastFertilizedDate: '2026-06-10',
        fertilizerIntervalDays: 14,
        lastRepottedDate: '2025-08-01',
        repotIntervalMonths: 12,
      ),
      Plant(
        id: '4',
        ...
        spaceId: 's1',
        journals: [],
        lastWateredDate: '2026-06-13',
        wateringIntervalDays: 2,
        lastFertilizedDate: '2026-06-11',
        fertilizerIntervalDays: 14,
      ...
      Plant(
        ...
        spaceId: 's3',
        journals: [],
        lastWateredDate: '2026-06-03',
        wateringIntervalDays: 14,
        lastFertilizedDate: '2026-05-15',
        fertilizerIntervalDays: 30,
        lastRepottedDate: '2024-06-01',
  ...
  void updatePlant(Plant p) {
    final idx = _plants.indexWhere((element) => element.id == p.id);
    if (idx != -1) {
      _plants[idx] = p;
      notifyListeners();
    }
  ...
  void recordWatering(String plantId, DateTime date) {
    final idx = _plants.indexWhere((p) => p.id == plantId);
    if (idx != -1) {
      final p = _plants[idx];
      final dateStr = "${date.year}-${date.month.toString().padLeft(2, '0')}-${date.day.toString().padLeft(2, '0')}";
      
      final updatedJournals = List<JournalEntry>.from(p.journals);
      updatedJournals.add(JournalEntry(
        id: 'j-${DateTime.now().millisecondsSinceEpoch}',
        date: dateStr,
        category: 'watering',
        title: 'Watered',
        notes: 'Watered via scheduler.',
      ));

      _plants[idx] = p.copyWith(
        lastWateredDate: dateStr,
        vitality: (p.vitality + 5).clamp(0, 100),
  ...
  void recordFertilization(String plantId, DateTime date) {
    final idx = _plants.indexWhere((p) => p.id == plantId);
    if (idx != -1) {
      final p = _plants[idx];
      final dateStr = "${date.year}-${date.month.toString().padLeft(2, '0')}-${date.day.toString().padLeft(2, '0')}";
      ...
      updatedJournals.add(JournalEntry(
        ...
        title: 'Fertilized',
        notes: 'Nutrients supplied.',
      ));

      _plants[idx] = p.copyWith(
        lastFertilizedDate: dateStr,
        vitality: (p.vitality + 8).clamp(0, 100),
        journals: updatedJournals,
      );
      notifyListeners();
    }
